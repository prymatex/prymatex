...
class ProfileManager(QtCore.QObject):
        ...
        if os.path.exists(self.profilesFile):
            ...
            for section in config.sections():
                if section.startswith("Profile"):
                    ...
                    self.profiles[profile.PMX_PROFILE_NAME] = profile
    ...
    def get_or_create_profile(self, name, path = None):
        ...
        if name in self.profiles:
            return self.profiles[name], False
        path = path if path is not None else os.path.abspath(os.path.join(self.PMX_HOME_PATH, name))
    ...
    def saveProfiles(self):
        ...
        for index, profile in enumerate(self.profiles.values()):
    ...
    def renameProfile(self, profile, newName):
        ...
        profile = self.profiles.pop(profile.PMX_PROFILE_NAME)

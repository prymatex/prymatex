...
def save_dictionary(data, filename):
    ...
    try:
        ...
        pickle.dump(data, file(pickle_filename, 'w'))
        tar = tarfile.open(filename, "w")
        for fname in [pickle_filename]+[fn for fn in saved_arrays.values()]:
...
def load_dictionary(filename):
    ...
    try:
        ...
        tar.extractall()
        pickle_filename = osp.splitext(filename)[0]+'.pickle'
        data = pickle.loads(file(pickle_filename, 'U').read())
        saved_arrays = {}
        if load_array is not None:
            # Loading numpy arrays saved with np.save
            try:
                saved_arrays = data.pop('__saved_arrays__')
                for (name, index), fname in saved_arrays.items():
...
class IOFunctions(object):
    ...
    def setup(self):
        ...
        load_filters.insert(0, str(_("Supported files")+" (*"+\
                                       " *".join(load_ext)+")"))
        self.load_filters = "\n".join(load_filters)
        self.save_filters = "\n".join(save_filters)

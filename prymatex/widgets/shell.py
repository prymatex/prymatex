...
from prymatex.qt.helpers.buttons import create_toolbutton
from prymatex.qt.helpers.actions import create_action
from prymatex.qt.helpers.menus import add_actions
from prymatex.qt.helpers.keysequences import keybinding
from prymatex.qt.helpers.keyevents import restore_keyevent

# Local import
from prymatex.core.config import get_conf_path, STDERR
from prymatex import resources
from prymatex.utils import encoding
from prymatex.utils.misc import get_error_match
from prymatex.utils.dochelpers import getobj

from prymatex.widgets.sourcecode.base import ConsoleBaseWidget
...
class ShellBaseWidget(ConsoleBaseWidget):
    ...
    def __init__(self, parent, history_filename, debug=False, profile=False):
        ...
        self.historylog_filename = get_conf_path('history.log')
        # Context menu
        self.menu = None
        ...
        self.__flushtimer = QtCore.QTimer(self)
        self.__flushtimer.setSingleShot(True)
        self.connect(self.__flushtimer, QtCore.SIGNAL('timeout()'), self.flush)
        ...
        #                                        completion_font)
        self.completion_widget.setup_appearance((10,10),
    ...
    def setup_context_menu(self):
        ...
        self.cut_action = create_action(self, { 'text': _("Cut"),
                                        'shortcut':keybinding('Cut'),
                                        'icon':resources.getIcon('editcut.png'),
                                        'triggered':self.cut })
        self.copy_action = create_action(self, { 'text':_("Copy"),
                                         'shortcut':keybinding('Copy'),
                                         'icon':resources.getIcon('editcopy.png'),
                                         'triggered':self.copy})
        paste_action = create_action(self, { 'text':_("Paste"),
                                     'shortcut':keybinding('Paste'),
                                     'icon':resources.getIcon('editpaste.png'),
                                     'triggered':self.paste})
        save_action = create_action(self, { 'text':_("Save history log..."),
                                    'icon':resources.getIcon('filesave.png'),
                                    'tip':_("Save current history log (i.e. all "
                                          "inputs and outputs) in a text file"),
                                    'triggered':self.save_historylog})
        self.delete_action = create_action(self, { 'text':_("Delete"),
                                    'shortcut':keybinding('Delete'),
                                    'icon':resources.getIcon('editdelete.png'),
                                    'triggered':self.delete})
        selectall_action = create_action(self, { 'text':_("Select All"),
                                    'shortcut':keybinding('SelectAll'),
                                    'icon':resources.getIcon('selectall.png'),
    ...
    input_buffer = QtCore.Property("QString", _get_input_buffer, _set_input_buffer)
    ...
    def copy(self):
        ...
        if self.has_selected_text():
            ...
            self.emit(QtCore.SIGNAL("keyboard_interrupt()"))
    ...
    def save_historylog(self):
        ...
        self.emit(QtCore.SIGNAL('redirect_stdio(bool)'), False)
        filename, _selfilter = getsavefilename(self, title,
                    self.historylog_filename, "%s (*.log)" % _("History logs"))
        self.emit(QtCore.SIGNAL('redirect_stdio(bool)'), True)
        if filename:
    ...
    def execute_command(self, command):
        self.emit(QtCore.SIGNAL("execute(QString)"), command)
    ...
    def preprocess_keyevent(self, event):
        ...
        if event.key() == QtCore.Qt.Key_C and ctrl:
            self.copy()
            event.accept()
            return True
        
        if self.new_input_line and ( len(event.text()) or event.key() in \
           (QtCore.Qt.Key_Up, QtCore.Qt.Key_Down, QtCore.Qt.Key_Left, QtCore.Qt.Key_Right) ):
            self.on_new_line()
    ...
    def postprocess_keyevent(self, event):
        ...
        elif key == QtCore.Qt.Key_Insert and not shift and not ctrl:
        ...
        elif key == QtCore.Qt.Key_Space and ctrl:
        ...
        elif (key == QtCore.Qt.Key_Home) or ((key == QtCore.Qt.Key_Up) and ctrl):
            self._key_home(shift)

        elif (key == QtCore.Qt.Key_End) or ((key == QtCore.Qt.Key_Down) and ctrl):
            self._key_end(shift)
        ...
        elif key == QtCore.Qt.Key_Escape and shift:
            self.clear_line()

        elif key == QtCore.Qt.Key_Escape:
            self._key_escape()
                
        elif key == QtCore.Qt.Key_L and ctrl:
            self.clear_terminal()
            
        elif key == QtCore.Qt.Key_V and ctrl:
            self.paste()
            
        elif key == QtCore.Qt.Key_X and ctrl:
            self.cut()
            
        elif key == QtCore.Qt.Key_Z and ctrl:
            self.undo()
            
        elif key == QtCore.Qt.Key_Y and ctrl:
            self.redo()
            
        elif key == QtCore.Qt.Key_A and ctrl:
            self.selectAll()
                
        elif key == QtCore.Qt.Key_Question and not self.has_selected_text():
            self._key_question(text)
            
        elif key == QtCore.Qt.Key_ParenLeft and not self.has_selected_text():
            self._key_parenleft(text)
            
        elif key == QtCore.Qt.Key_Period and not self.has_selected_text():
    ...
    def add_to_history(self, command):
        ...
        self.emit(QtCore.SIGNAL('append_to_history(QString,QString)'),
    ...
    def mousePressEvent(self, event):
        ...
        event: the mouse press event (QtGui.QMouseEvent)
        """
        if event.button() == QtCore.Qt.MidButton:
            text = self.get_selected_text()
            # Simulating left mouse button:
            event = QtGui.QMouseEvent(QtGui.QMouseEvent.MouseButtonPress, event.pos(),
...
class PythonShellWidget(ShellBaseWidget):
    ...
    def __init__(self, parent, history_filename, debug=False, profile=False):
        ...
        # Local shortcuts
        self.inspectsc = QtGui.QShortcut(QtGui.QKeySequence("Ctrl+I"), self,
                                   self.inspect_current_object)
        self.inspectsc.setContext(QtCore.Qt.WidgetWithChildrenShortcut)
        
    def get_shortcut_data(self):
        """
        Returns shortcut data, a list of tuples (shortcut, text, default)
        shortcut (QtGui.QShortcut or QAction instance)
    ...
    def setup_context_menu(self):
        ...
        self.copy_without_prompts_action = create_action(self,
                                     { 'text':_("Copy without prompts"),
                                     'icon':resources.getIcon('copywop.png'),
                                     'triggered':self.copy_without_prompts})
        clear_line_action = create_action(self, { 'text':_("Clear line"),
                                     'shortcut': QtGui.QKeySequence("Shift+Escape"),
                                     'icon':resources.getIcon('eraser.png'),
                                     'tip':_("Clear line"),
                                     'triggered':self.clear_line})
        clear_action = create_action(self, { 'text':_("Clear shell"),
                                     'shortcut': QtGui.QKeySequence("Ctrl+L"),
                                     'icon':resources.getIcon('clear.png'),
    ...
    def mouseReleaseEvent(self, event):
        ...
        if get_error_match(text) and not self.has_selected_text():
            self.emit(QtCore.SIGNAL("go_to_error(QString)"), text)

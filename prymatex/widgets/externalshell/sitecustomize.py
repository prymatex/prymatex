...
# Prepending this prymatex package's path to sys.path to be sure 
# that another version of prymatex won't be imported instead:
prymatex_path = osp.dirname(__file__)
while not osp.isdir(osp.join(prymatex_path, 'prymatex')):
    prymatex_path = osp.abspath(osp.join(prymatex_path, os.pardir))
if not prymatex_path.startswith(sys.prefix):
    # Prymatex is not installed: moving its parent directory to the top of 
    # sys.path to be sure that this prymatex package will be imported in 
    # the remote process (instead of another installed version of Prymatex)
    while prymatex_path in sys.path:
        sys.path.remove(prymatex_path)
    sys.path.insert(0, prymatex_path)
os.environ['PRYMATEX_PARENT_DIR'] = prymatex_path
...
if os.environ.get("MATPLOTLIB_PATCH", "").lower() == "true":
    try:
        from prymatex.utils import mpl_patch
        mpl_patch.apply()
    except ImportError:
        pass
...
os.environ['PRYMATEX_ENCODING'] = encoding
...
# Communication between Prymatex and the remote process
if os.environ.get('PRYMATEX_SHELL_ID') is None:
    monitor = None
else:
    from prymatex.widgets.externalshell.monitor import Monitor
    monitor = Monitor("127.0.0.1",
                      int(os.environ['PRYMATEX_I_PORT']),
                      int(os.environ['PRYMATEX_N_PORT']),
                      os.environ['PRYMATEX_SHELL_ID'],
                      float(os.environ['PRYMATEX_AR_TIMEOUT']),
                      os.environ["PRYMATEX_AR_STATE"].lower() == "true")
    monitor.start()
    
    def open_in_prymatex(source, lineno=1):
        """Open in Prymatex's editor the source file
(may be a filename or a Python module/package)"""
    ...
    __builtin__.open_in_prymatex = open_in_prymatex
    ...
    if os.environ.get("INSTALL_QT_INPUTHOOK", "").lower() == "true"\
       and not os.environ.get('IPYTHON', False):
        # For now, the Prymatex's input hook does not work with IPython:
        ...
        def qt_inputhook():
            """Qt input hook for Prymatex's console
            ...
            # (otherwise, on slow machines, this may freeze Prymatex)
...
#                socket.connectToServer(os.environ['PRYMATEX_SHELL_ID'])
...
#                socket.disconnectFromServer()
        ...
        # Installing Prymatex's PyOS_InputHook implementation:
        import ctypes
...
# Monkey-patching pdb
#===============================================================================
class PrymatexPdb(pdb.Pdb):
    def set_prymatex_breakpoints(self):
    ...
    def notify_prymatex(self, frame):
        if not frame:
            return
...
def interaction(self, frame, traceback):
    ...
    self.notify_prymatex(frame) #-----Prymatex-specific-------------------------
...
def reset(self):
    ...
    self.set_prymatex_breakpoints()

#XXX: notify prymatex on any pdb command (is that good or too lazy? i.e. is more 
#     specific behaviour desired?)
@monkeypatch_method(pdb.Pdb, 'Pdb')
def postcmd(self, stop, line):
    self.notify_prymatex(self.curframe)
    return self._old_Pdb_postcmd(stop, line)


# Restoring (almost) original sys.path:
# (Note: do not remove prymatex_path from sys.path because if Prymatex has been
#  installed using python setup.py install, then this could remove the 
#  'site-packages' directory from sys.path!)
try:
    sys.path.remove(osp.join(prymatex_path,
                             "prymatex", "widgets", "externalshell"))
except ValueError:
    pass
    ...
    try:
        ...
        def patched_setapi(name, no):
            try:
                ...
                print >>sys.stderr, "Warning/PyQt4-Prymatex (%s)" % str(msg)
...
class UserModuleDeleter(object):
    ...
    def __init__(self, namelist=None, pathlist=None):
        if namelist is None:
            namelist = []
        self.namelist = namelist + ['prymatex']
        if pathlist is None:
            pathlist = []
...
def evalsc(command):
    ...
    if system_command:
        ...
        if cd_match:
            ...
            from prymatex.conre import config
            execfile(config.SCIENTIFIC_STARTUP, namespace)
        else:

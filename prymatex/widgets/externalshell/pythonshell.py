...
from prymatex.qt.helpers.icons import get_std_icon
from prymatex.qt.helpers.dialogs import DialogManager

#from spyderlib.utils.environ import RemoteEnvDialog
from prymatex.utils.programs import get_python_args
from prymatex.utils.misc import get_python_executable
from prymatex.core.config import _, get_module_source_path
from prymatex import resources
from prymatex.widgets.shell import PythonShellWidget
#from spyderlib.widgets.externalshell.namespacebrowser import NamespaceBrowser
from prymatex.widgets.externalshell.monitor import communicate, write_packet
from prymatex.widgets.externalshell.baseshell import (ExternalShellBase,
...
class ExtPythonShellWidget(PythonShellWidget):
    ...
    def clear_terminal(self):
        ...
        self.clear()
        self.emit(QtCore.SIGNAL("execute(QString)"), "\n")
    ...
    def execute_lines(self, lines):
        ...
        for line in lines.splitlines():
            ...
            if sys.platform.startswith("darwin"):
                ...
                self.emit(QtCore.SIGNAL("wait_for_ready_read()"))
            self.flush()
    ...
    def __init__(self, parent=None, fname=None, wdir=None,
        ...
        if self.is_ipython_shell or self.is_ipython_kernel:
            ...
            self.fname = get_module_source_path(
                            'prymatex.widgets.externalshell', 'startup.py')
...
(it is given the chance to prompt the user for any unsaved files, etc).""") })
    ...
    def get_options_menu(self):
        ...
        self.interact_action = create_action(self, { 'text': _("Interact") })
        self.interact_action.setCheckable(True)
        self.debug_action = create_action(self, { 'text': _("Debug") })
        self.debug_action.setCheckable(True)
        self.args_action = create_action(self, { 'text': _("Arguments..."),
                                         'triggered':self.get_arguments })
        run_settings_menu = QtGui.QMenu(_("Run settings"), self)
        add_actions(run_settings_menu,
                    (self.interact_action, self.debug_action, self.args_action))
        self.cwd_button = create_action(self, { 'text':_("Working directory"),
                                'icon':get_std_icon('DirOpenIcon'),
                                'tip':_("Set current working directory"),
                                'triggered':self.set_current_working_directory })
        self.env_button = create_action(self, { 'text':_("Environment variables"),
                                        'icon':resources.getIcon('environ.png'),
                                        'triggered':self.show_env })
        self.syspath_button = create_action(self,
                                            { 'text':_("Show sys.path contents"),
                                            'icon':resources.getIcon('syspath.png'),
    ...
    def get_shell_widget(self):
        if self.stand_alone is None:
            ...
            self.connect(self.namespacebrowser, QtCore.SIGNAL('collapse()'),
                         lambda: self.toggle_globals_explorer(False))
            # Shell splitter
            self.splitter = splitter = QtGui.QSplitter(QtCore.Qt.Vertical, self)
            self.connect(self.splitter, QtCore.SIGNAL('splitterMoved(int, int)'),
                         self.splitter_moved)
    ...
    def get_icon(self):
        return resources.getIcon('python.png')
    ...
    def configure_namespacebrowser(self):
        ...
        if self.notification_thread is not None:
            self.connect(self.notification_thread,
                         QtCore.SIGNAL('refresh_namespace_browser()'),
    ...
    def create_process(self):
        ...
        self.connect(self.shell, QtCore.SIGNAL("wait_for_ready_read()"),
        ...
        if self.monitor_enabled:
            env.append('PRYMATEX_SHELL_ID=%s' % id(self))
            env.append('PRYMATEX_AR_TIMEOUT=%d' % self.autorefresh_timeout)
            env.append('PRYMATEX_AR_STATE=%r' % self.autorefresh_state)
            from prymatex.widgets.externalshell import introspection
            ...
            self.connect(self.notification_thread, QtCore.SIGNAL('pdb(QString,int)'),
                         lambda fname, lineno:
                         self.emit(QtCore.SIGNAL('pdb(QString,int)'), fname, lineno))
            self.connect(self.notification_thread,
                         QtCore.SIGNAL('new_ipython_kernel(QString)'),
                         lambda args:
                         self.emit(QtCore.SIGNAL('create_ipython_frontend(QString)'),
                         args))
            self.connect(self.notification_thread,
                         QtCore.SIGNAL('open_file(QString,int)'),
                         lambda fname, lineno:
                         self.emit(QtCore.SIGNAL('open_file(QString,int)'),
                                   fname, lineno))
            if self.namespacebrowser is not None:
                self.configure_namespacebrowser()
            env.append('PRYMATEX_I_PORT=%d' % introspection_server.port)
            env.append('PRYMATEX_N_PORT=%d' % notification_server.port)
...
#            self.local_server.listen(str(id(self)))
        ...
        self.connect(self.process, QtCore.SIGNAL("readyReadStandardError()"),
                     self.write_error)
        self.connect(self.process, QtCore.SIGNAL("finished(int,QProcess::ExitStatus)"),
                     self.finished)
                     
        self.connect(self, QtCore.SIGNAL('finished()'), self.dialog_manager.close_all)

        self.connect(self.terminate_button, QtCore.SIGNAL("clicked()"),
                     self.process.terminate)
        self.connect(self.kill_button, QtCore.SIGNAL("clicked()"),
                     self.process.kill)

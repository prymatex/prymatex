...
from prymatex.core.config import get_conf_path
...
class ExternalShellBase(QtGui.QWidget):
    ...
    def __init__(self, parent=None, fname=None, wdir=None,
        ...
        self.shell = self.SHELL_CLASS(parent, history_filename)
        #self.shell = self.SHELL_CLASS(parent, get_conf_path(history_filename))
        self.shell.set_light_background(light_background)
        self.connect(self.shell, QtCore.SIGNAL("execute(QString)"),
                     self.send_to_process)
        self.connect(self.shell, QtCore.SIGNAL("keyboard_interrupt()"),
                     self.keyboard_interrupt)
        # Redirecting some SIGNALs:
        self.connect(self.shell, QtCore.SIGNAL('redirect_stdio(bool)'),
                     lambda state: self.emit(QtCore.SIGNAL('redirect_stdio(bool)'),
                                             state))
    ...
    def get_toolbar_buttons(self):
        if self.run_button is None:
            self.run_button = create_toolbutton(self, { 'text':_("Run"),
                                             'icon':resources.getIcon('run.png'),
        ...
        if self.kill_button is None:
            self.kill_button = create_toolbutton(self, { 'text':_("Kill"),
                                     'icon':resources.getIcon('kill.png'),
        ...
        if self.options_button is None:
            ...
            if options:
                self.options_button = create_toolbutton(self, { 'text':_("Options"),
                                            'icon':resources.getIcon('tooloptions.png') })
                self.options_button.setPopupMode(QtGui.QToolButton.InstantPopup)
    ...
    def get_options_menu(self):
        self.show_time_action = create_action(self, { 'text': _("Show elapsed time"),
    ...
    def closeEvent(self, event):
        ...
        self.disconnect(self.timer, QtCore.SIGNAL("timeout()"), self.show_time)
    ...
    def set_running_state(self, state=True):
        ...
        if state:
            ...
            self.t0 = time()
            self.connect(self.timer, QtCore.SIGNAL("timeout()"), self.show_time)
    ...
    def get_arguments(self):
        arguments, valid = QtGui.QInputDialog.getText(self, _('Arguments'),
...
def test():
    ...
    #shell = ExternalPythonShell(wdir=osp.dirname(prymatex.__file__),
    #                            ipython_kernel=True, 
    ...
    #shell = ExternalSystemShell(wdir=osp.dirname(prymatex.__file__),
    #                            light_background=False)
    shell.shell.toggle_wrap_mode(True)
    shell.start_shell(False)
    from prymatex.qt.QtGui import QFont

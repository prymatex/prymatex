...
# Local imports
from prymatex.utils.misc import fix_reference_name
from prymatex.utils.debug import log_last_error
from prymatex.utils.dochelpers import (getargtxt, getdoc, getsource,
                                        getobjdir, isdefined)
from prymatex.utils.bsdsocket import (communicate, read_packet, write_packet,
                                       PACKET_NOT_RECEIVED)
from prymatex.utils.module_completion import moduleCompletion
from prymatex.core.config import get_conf_path, get_supported_types, DEBUG
...
def get_remote_data(data, settings, mode, more_excluded_names=None):
    ...
    """
    from prymatex.widgets.dicteditorutils import globalsfilter
...
def make_remote_view(data, settings, more_excluded_names=None):
    ...
    -> globals explorer
    """
    from prymatex.widgets.dicteditorutils import (get_human_readable_type,
...
class Monitor(threading.Thread):
    ...
    def saveglobals(self):
        ...
        ns = self.get_current_namespace()
        from prymatex.utils.iofuncs import iofunctions

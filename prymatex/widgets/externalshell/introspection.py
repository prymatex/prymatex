...
# Local imports
from prymatex.core.config import get_conf_path, DEBUG
from prymatex.utils.misc import select_port
from prymatex.utils.debug import log_last_error
from prymatex.utils.bsdsocket import read_packet, write_packet
...
class IntrospectionServer(threading.Thread):
    ...
    def __init__(self):
        ...
        self.shells = {}
        self.setDaemon(True)
        global PRYMATEX_PORT
        self.port = PRYMATEX_PORT = select_port(default_port=PRYMATEX_PORT)
        PRYMATEX_PORT += 1
...
class NotificationThread(QtCore.QThread):
    """Notification thread"""
    sig_process_remote_view = QtCore.Signal(object)
    def __init__(self):
        ...
        while True:
            ...
            try:
                ...
                if command == 'pdb_step':
                    ...
                    self.emit(QtCore.SIGNAL('refresh_namespace_browser()'))
                elif command == 'refresh':
                    self.emit(QtCore.SIGNAL('refresh_namespace_browser()'))
                elif command == 'remote_view':
                    self.sig_process_remote_view.emit(data)
                elif command == 'ipython_kernel':
                    self.emit(QtCore.SIGNAL('new_ipython_kernel(QString)'), data)
                elif command == 'open_file':
                    fname, lineno = data
                    self.emit(QtCore.SIGNAL('open_file(QString,int)'), fname, lineno)
                else:

...
import os, re, string, sre_constants

from prymatex.qt import QtGui, QtCore

# Local imports
from prymatex.widgets.sourcecode.terminal import ANSIEscapeCodeHandler
from prymatex.utils import sourcecode

class CompletionWidget(QtGui.QListWidget):
    """Completion list widget"""
    def __init__(self, parent, ancestor):
        QtGui.QListWidget.__init__(self, ancestor)
        ...
        self.hide()
        self.connect(self, QtCore.SIGNAL("itemActivated(QListWidgetItem*)"),
                     self.item_selected)
...
class TextEditBaseWidget(QtGui.QPlainTextEdit):
    """
    ...
    def __init__(self, parent=None):
        ...
        if parent is not None:
            mainwin = parent
            while not isinstance(mainwin, QtGui.QMainWindow):
        ...
        self.calltip_font = QtGui.QFont()
        self.completion_text = ""

        # Brace matching
        self.bracepos = None
        self.matched_p_color = QtGui.QColor(QtCore.Qt.green)
        self.unmatched_p_color = QtGui.QColor(QtCore.Qt.red)
    ...
    def changed(self):
        """Emit changed QtCore.SIGNAL"""
        self.emit(QtCore.SIGNAL('modificationChanged(bool)'),
    ...
    def __highlight(self, positions, color=None, cancel=False):
        ...
        for position in positions:
            ...
            selection.cursor.movePosition(QtGui.QTextCursor.NextCharacter,
    ...
    def set_wrap_mode(self, mode=None):
        ...
        """
        if mode == 'word':
            wrap_mode = QtGui.QTextOption.WrapAtWordBoundaryOrAnywhere
        elif mode == 'character':
            wrap_mode = QtGui.QTextOption.WrapAnywhere
        else:
            wrap_mode = QtGui.QTextOption.NoWrap
    ...
    def move_cursor(self, chars=0):
        ...
        direction = QtGui.QTextCursor.Right if chars > 0 else QtGui.QTextCursor.Left
        for _i in range(abs(chars)):
            self.moveCursor(direction, QtGui.QTextCursor.MoveAnchor)
    ...
    def __move_cursor_anchor(self, what, direction, move_mode):
        ...
        if what == 'character':
            if direction == 'left':
                self.moveCursor(QtGui.QTextCursor.PreviousCharacter, move_mode)
            elif direction == 'right':
                self.moveCursor(QtGui.QTextCursor.NextCharacter, move_mode)
        elif what == 'word':
            if direction == 'left':
                self.moveCursor(QtGui.QTextCursor.PreviousWord, move_mode)
            elif direction == 'right':
                self.moveCursor(QtGui.QTextCursor.NextWord, move_mode)
        elif what == 'line':
            if direction == 'down':
                self.moveCursor(QtGui.QTextCursor.NextBlock, move_mode)
            elif direction == 'up':
                self.moveCursor(QtGui.QTextCursor.PreviousBlock, move_mode)
    ...
    def move_cursor_to_next(self, what='word', direction='left'):
        ...
        """
        self.__move_cursor_anchor(what, direction, QtGui.QTextCursor.MoveAnchor)
    ...
    def extend_selection_to_next(self, what='word', direction='left'):
        ...
        toward *direction* ('left' or 'right')
        """
        self.__move_cursor_anchor(what, direction, QtGui.QTextCursor.KeepAnchor)
    ...
    def copy(self):
        ...
        """
        QtGui.QApplication.clipboard().setText(self.get_selected_text())
    ...
    def find_multiline_pattern(self, regexp, cursor, findflag):
        """Reimplement QtGui.QTextDocument's find method
        ...
        try:
            ...
            return
        if findflag & QtGui.QTextDocument.FindBackward:
            # Find backward
        ...
        if match:
            ...
            fcursor.setPosition(pos2, QtGui.QTextCursor.KeepAnchor)
            return fcursor
    ...
    def find_text(self, text, changed=True, forward=True, case=False,
        ...
        """Find text"""
        cursor = self.textCursor()
        findflag = QtGui.QTextDocument.FindFlag()
        if not forward:
            findflag = findflag | QtGui.QTextDocument.FindBackward
        moves = [QtGui.QTextCursor.NoMove]
        if forward:
            moves += [QtGui.QTextCursor.NextWord, QtGui.QTextCursor.Start]
            if changed:
            ...
            moves += [QtGui.QTextCursor.End]
        if not regexp:
            text = re.escape(unicode(text))
        pattern = QtCore.QRegExp(r"\b%s\b" % text if words else text,
                          QtCore.Qt.CaseSensitive if case else QtCore.Qt.CaseInsensitive,
        ...
        for move in moves:
            ...
            if regexp and '\\n' in text:
                ...
                # Single line find: using the QtGui.QTextDocument's find function,
    ...
    def get_current_word(self):
        ...
        if cursor.hasSelection():
            ...
            def is_space(move):
                ...
                curs.movePosition(move, QtGui.QTextCursor.KeepAnchor)
                return not unicode(curs.selectedText()).strip()
            if is_space(QtGui.QTextCursor.NextCharacter):
                if is_space(QtGui.QTextCursor.PreviousCharacter):
                    return
    ...
    def get_line_number_at(self, coordinates):
        """Return line number at *coordinates* (QtCore.QPoint)"""
        cursor = self.cursorForPosition(coordinates)
        return cursor.blockNumber()-1
    
    def get_line_at(self, coordinates):
        """Return line at *coordinates* (QtCore.QPoint)"""
        cursor = self.cursorForPosition(coordinates)
        cursor.select(QtGui.QTextCursor.BlockUnderCursor)
        return unicode(cursor.selectedText()).replace(u'\u2029', '')
    
    def get_word_at(self, coordinates):
        """Return word at *coordinates* (QtCore.QPoint)"""
        cursor = self.cursorForPosition(coordinates)
        cursor.select(QtGui.QTextCursor.WordUnderCursor)
        return unicode(cursor.selectedText())
    ...
    def __duplicate_line_or_selection(self, after_current_line=True):
        ...
        cursor.setPosition(orig_sel[1], QtGui.QTextCursor.KeepAnchor)
        self.setTextCursor(cursor)
    ...
    def show_calltip(self, title, text, color='#2D62FF',
        ...
        point = self.mapToGlobal(QtCore.QPoint(cx, cy))
        point.setX(point.x()+self.get_linenumberarea_width())
        QtGui.QToolTip.showText(point, tiptext)
    ...
    def hide_tooltip_if_necessary(self, key):
        ...
        try:
            ...
            before = self.is_cursor_before(self.calltip_position,
                                           char_offset=1)
            other = key in (QtCore.Qt.Key_ParenRight, QtCore.Qt.Key_Period, QtCore.Qt.Key_Tab)
    ...
    def __get_move_mode(self, shift):
        return QtGui.QTextCursor.KeepAnchor if shift else QtGui.QTextCursor.MoveAnchor

    def stdkey_up(self, shift):
        self.moveCursor(QtGui.QTextCursor.Up, self.__get_move_mode(shift))

    def stdkey_down(self, shift):
        self.moveCursor(QtGui.QTextCursor.Down, self.__get_move_mode(shift))
    ...
    def stdkey_end(self, shift):
        self.moveCursor(QtGui.QTextCursor.EndOfBlock, self.__get_move_mode(shift))
    ...
    def focusInEvent(self, event):
        ...
        self.emit(QtCore.SIGNAL("focus_changed()"))
        self.emit(QtCore.SIGNAL("focus_in()"))
...
class QtANSIEscapeCodeHandler(ANSIEscapeCodeHandler):
    ...
    def set_style(self):
        ...
        if self.current_format is None:
            ...
            self.current_format = QtGui.QTextCharFormat(self.base_format)
        # Foreground color
...
class ConsoleBaseWidget(TextEditBaseWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.connect(self, QtCore.SIGNAL('userListActivated(int, const QString)'),
                     lambda user_id, text:
                     self.emit(QtCore.SIGNAL('completion_widget_activated(QString)'),
                               text))
    ...
    def set_light_background(self, state):
        ...
        if state:
            self.set_palette(background=QtGui.QColor(QtCore.Qt.white),
                             foreground=QtGui.QColor(QtCore.Qt.darkGray))
        else:
            self.set_palette(background=QtGui.QColor(QtCore.Qt.black),
                             foreground=QtGui.QColor(QtCore.Qt.lightGray))
    ...
    def paste(self):
        ...
        if self.has_selected_text():
            self.remove_selected_text()
        self.insert_text(QtGui.QApplication.clipboard().text())
    ...
    def append_text_to_shell(self, text, error, prompt):
        ...
        if error:
            ...
            if is_traceback:
                self.emit(QtCore.SIGNAL('traceback_available()'))
        elif prompt:

...
class Qt(object):
    ...
    Key_Pause=0x01000008 #The Pause/Break key (Note: Not anything to do with pausing media)
    ...
    SHIFT = Key_Shift=0x01000020	 
    CTRL = Key_Control=0x01000021 #On Mac OS X, this corresponds to the Command keys.
    META = Key_Meta=0x01000022 #On Mac OS X, this corresponds to the Control keys. On Windows keyboards, this key is mapped to the Windows key.
    ALT = Key_Alt=0x01000023
    ALTGR = Key_AltGr=0x01001103 #On Windows, when the KeyDown event for this key is sent, the Ctrl+Alt modifiers are also set.
    ...
    Key_Book=0x010000c9	 
    Key_CD=0x010000ca	 
    Key_Calculator=0x010000cb #On X11 this key is not mapped for legacy reasons. Use Key_Launch1 instead.
    ...
    Key_Call=0x01100004	#A key to answer or initiate a call (see Key_ToggleCallHangup for a key to toggle current call state)
    ...
    Key_Flip=0x01100006	 
    Key_Hangup=0x01100005	#A key to end an ongoing call (see Key_ToggleCallHangup for a key to toggle current call state)
    Key_No=0x01010002	 
...
class QKeySequence(Mock):
    ...
    def __str__(self):
        string = []
        if self.code & Qt.SHIFT:
            string.append("Shift")
        if self.code & Qt.CTRL:
            string.append("Ctrl")
        if self.code & Qt.META:
            string.append("Meta")
        if self.code & Qt.ALT:
            string.append("Alt")
        if self.code & Qt.ALTGR:
            string.append("Altgr")
        string.append(chr(self.code & 0xff))
        return '+'.join(string)

QTextCharFormat = QColor = QFont = Mock